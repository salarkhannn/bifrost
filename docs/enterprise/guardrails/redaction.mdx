...
Use `runtime_reversible` when runtime content should be redacted, but authorized users still need an audit path back to the original values in Bifrost logs.
...
Users need the `Logs:Reveal` permission to reveal original values for a log that has reversible redaction data. The reveal API returns the placeholder mapping for that log, for example:
